...
# -------------------- V2: LangGraph-powered endpoints --------------------
...
def v2_process():
	if 'image' not in request.files:
		return jsonify({'error': 'No image file provided'}), 400
	image_file = request.files['image']
	id_face_file = request.files.get('id_face')
	...
	with tempfile.TemporaryDirectory() as temp_dir:
		...
		selfie_path = None
		if id_face_file:
			id_face_path = os.path.join(temp_dir, 'id_face.jpg')
			id_face_file.save(id_face_path)
		if selfie_file:
			selfie_path = os.path.join(temp_dir, 'selfie.jpg')
			selfie_file.save(selfie_path)

		structured, report, kyc_status = run_pipeline(img_path, id_face_path, selfie_path)
		return jsonify({
